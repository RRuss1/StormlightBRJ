    ...
    case 'state_sync':
      ...
      if (playersChanged || phaseChanged) {
        ...
        if (isOnCombat) {
          ...
          loadLog(false).then((l) => {
            renderAll(l);
            setBottomFromState(l);
          });
        } else {
          renderPartyStrip();
          setBottomFromState();
        } // fast path: only HP/focus changed
      }
      break;
    case 'slot_assigned':
...
function showActionBadge(playerName) {
  ...
  badge.style.cssText =
    'position:absolute;top:4px;right:4px;font-size:9px;color:var(--teal2);font-family:var(--font-d);letter-spacing:1px;';
...
function showToast(message) {
  ...
  toast.style.cssText =
    'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--bg3);border:1px solid var(--border2);border-radius:20px;padding:8px 20px;font-family:var(--font-d);font-size:11px;letter-spacing:1px;color:var(--text3);z-index:9999;pointer-events:none;animation:stormReveal 0.3s ease-out;';
...
function getAct(m) {
  return ACTS.find((a) => m >= a.start && m <= a.end) || ACTS[0];
}
function getSprenStage(m) {
  if (m < 20) return 0;
  if (m < 50) return 1;
  if (m < 90) return 2;
  if (m < 140) return 3;
...
async function listCampaigns() {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    try {
      ...
      // Track owner_id for ownership checks
      if (row.owner_id) window._campaignOwners[row.id] = row.owner_id;
    } catch (e) {
      camps.push({ id: row.id, name: row.name, state: null });
    }
  }
  return camps;
}
...
async function ensureSheets() {
  ...
  if (!(await loadState())) {
    await saveState({
      players: new Array(partySize).fill(null),
      turn: 0,
      totalMoves: 0,
      phase: 'pregame',
      partySize,
...
function renderCampaigns(camps) {
  ...
  grid.innerHTML =
    filtered
      ...
      .join('') +
    `<div class="camp-card new-camp" onclick="promptNewCampaign()" style="border-color:${themeColor}22;"><div class="camp-new-icon" style="color:${themeColor};">+</div><div class="camp-new-txt" style="color:${themeColor};">New Campaign</div></div>`;
  if (lang === 'th') setTimeout(applyThaiToPage, 100);
}
function promptNewCampaign(num) {
  pendingCampNum = num || null; // num no longer used for ID
  const m = document.getElementById('new-camp-modal');
  const inp = document.getElementById('camp-name-input');
...
async function confirmNewCampaign() {
  ...
  if (!name) {
    const e = document.getElementById('new-camp-err');
    e.textContent = 'Give your campaign a name first.';
...
async function _doSelectCampaign(id) {
  ...
  try {
    ...
    if (gState && gState.phase === 'pregame' && myChar) {
      showScreen('lobby');
      renderLobby();
      startLobbyPolling();
      return;
    ...
    if (enterBtn)
      enterBtn.querySelector('span').textContent = gState && gState.phase === 'playing' ? '⟁ Rejoin the Storm' : '⟁ Create Character';
    showScreen('title');
    if (status) status.textContent = '';
    document.querySelectorAll('.camp-card').forEach((card) => (card.style.opacity = '1'));
  } catch (e) {
...
function _getOwnedCampaigns() {
  try {
    return JSON.parse(localStorage.getItem('cyoa_owned_campaigns') || '{}');
  } catch (e) {
    return {};
  }
}
function _ownCampaign(id) {
  // localStorage (legacy/guest)
  const owned = _getOwnedCampaigns();
  owned[id] = Date.now();
  localStorage.setItem('cyoa_owned_campaigns', JSON.stringify(owned));
  // DB: set owner_id on campaign if logged in
  if (window.Auth && window.Auth.isLoggedIn()) {
    window.Auth.claimOwnership([id], []).catch(() => {});
  }
}
function _isOwnedCampaign(id) {
  // DB-first: if logged in, check if user is in campaign_members as owner or created it
  if (window.Auth && window.Auth.isLoggedIn()) {
    const user = window.Auth.getCurrentUser();
    if (user) {
      // Check gState for ownership (creator's userId stamped on players)
      if (gState && gState.players) {
        const myPlayer = gState.players.find((p) => p && p.userId === user.id && !p.isNPC);
        if (myPlayer) return true;
      }
      // Check campaign metadata cache
      if (window._campaignOwners && window._campaignOwners[id] === user.id) return true;
    }
  }
  // Fallback: localStorage (guest/legacy)
...
async function shareCampaign(campId, worldId) {
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      toast.style.cssText =
        'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--bg3);border:1px solid var(--primary,var(--gold));color:var(--primary,var(--gold));padding:8px 20px;border-radius:20px;font-family:var(--font-d);font-size:11px;letter-spacing:2px;z-index:999;';
      document.body.appendChild(toast);
...
function renderPSZ() {
  ...
  document.getElementById('psz-row').innerHTML = [2, 3, 4, 5]
    .map(
      (n) =>
        `<div class="psz${partySize === n ? ' sel' : ''}${locked ? ' locked' : ''}" onclick="${locked ? '' : `pickSize(${n})`}">${n} Players</div>`
    )
    .join('');
...
function pickSize(n) {
  ...
  partySize = n;
  if (gState && gState.phase === 'pregame') {
    gState.players = Array.from({ length: n }, (_, i) => gState.players[i] || null);
    gState.partySize = n;
...
function genNPC(slot) {
  ...
  const _npcDefs = window.ConfigResolver
    ...
    : {
        physDef: 10 + Math.floor(((stats.str || 0) + (stats.spd || 0)) / 2),
...
async function onEnter() {
  ...
  try {
    ...
    if (myChar && gState && gState.phase === 'pregame') {
      mySlot = myChar.slot ?? mySlot;
      showScreen('lobby');
      renderLobby();
      startLobbyPolling();
    ...
    if (slotIdx === -1) {
      setTitleStatus('All slots are filled or being claimed.');
      return;
    }
    setTitleStatus('');
    showScreen('create');
...
async function claimNextSlot() {
  ...
  try {
    ...
    if (fresh) {
      gState = fresh;
      partySize = fresh.partySize || partySize;
    }
    if (!gState) {
      gState = {
        players: new Array(partySize).fill(null),
...
async function reserveSlotOnly(i) {
  ...
  try {
    ...
    console.warn('reserveSlot save failed:', e);
  }
}
...
function setTitleStatus(m) {
  const el = document.getElementById('title-status');
...
function renderCreate() {
  ...
  const paths = cc.paths || [
    { id: 'class', label: 'Class', icon: '⚔', desc: '', sublabel: '' },
    { id: 'background', label: 'Background', icon: '✦', desc: '', sublabel: '' },
  ];
  ...
  if (originGrid) {
    ...
    if (origins && origins.length) {
      originGrid.innerHTML = origins
        .map((o) => `<button class="origin-btn" onclick="pickOrigin(this,'${o.replace(/'/g, "\\'")}')"> ${o}</button>`)
        .join('');
...
function createNext() {
  ...
  if (createStep === 1) {
    ...
    if (isRadiant && !selClass) {
      if (err) {
        err.textContent = 'Choose your ' + _classLabel + '.';
        err.style.display = 'block';
      }
      return;
    }
    if (!isRadiant && !selRole) {
      if (err) {
        err.textContent = 'Choose your ' + _roleLabel + '.';
        err.style.display = 'block';
      }
      return;
    }
    ...
    if (!selColor) {
      if (err) {
        err.textContent = 'Choose your color.';
        err.style.display = 'block';
      }
      return;
    }
...
function pickCharType(radiant) {
  ...
  if (r) {
    r.classList.toggle('sel', radiant);
  }
  if (h) {
    h.classList.toggle('sel', !radiant);
  }
}
...
function renderColors() {
  ...
  document.getElementById('color-row').innerHTML = COLORS.map(
    (c) =>
      `<div class="cswatch${selColor === c.hex ? ' sel' : ''}" style="background:${c.hex};opacity:${taken.includes(c.hex) ? 0.2 : 1};" title="${c.name}" onclick="pickColor('${c.hex}',${taken.includes(c.hex)})"></div>`
  ).join('');
}
function pickColor(h, t) {
  if (t) return;
  selColor = h;
  renderColors();
  updateCreateSubmitBtn();
...
function pickClass(id) {
  selClass = CLASSES.find((c) => c.id === id);
  renderClasses();
  rollStats();
  renderStatsPointBuy();
  updateCreateSubmitBtn();
...
function renderRoles() {
  ...
  grid.innerHTML = HERO_ROLES.map(
    ...
    <div style="font-size:11px;color:var(--text5);margin-top:4px;">Attr bonus: ${
...
function pickRole(id) {
  selRole = HERO_ROLES.find((r) => r.id === id);
  renderRoles();
  rollStats();
  renderStatsPointBuy();
  updateCreateSubmitBtn();
...
function renderWeapons() {
  ...
  if (!grid) return;
  grid.innerHTML = HERO_WEAPONS.map(
    (w) =>
      `<div class="ccard${selWeapon && selWeapon.id === w.id ? ' sel' : ''}" onclick="pickWeapon('${w.id}')"><div class="ccard-name">${w.name}</div><div class="ccard-ideal">${w.type}</div><div class="ccard-desc">${w.desc}</div><div class="ccard-bonus">DMG: Crit+${w.dmgBonus.crit} Hit+${w.dmgBonus.hit}</div></div>`
  ).join('');
}
function pickWeapon(id) {
  selWeapon = HERO_WEAPONS.find((w) => w.id === id);
  renderWeapons();
}
...
function getPointsSpent() {
  return Object.values(_pbAlloc).reduce((a, v) => a + v, 0);
}
function getPointsLeft() {
  return ATTR_POINTS_START - getPointsSpent();
}
...
function r4d6() {
  ...
  d.sort((a, b) => a - b);
  return d.slice(1).reduce((a, v) => a + v, 0);
}
function modStr(v) {
  return '+' + (v || 0);
} // Official: attribute score IS the modifier
...
async function onCreateChar() {
  ...
  try {
    ...
    if (isRadiant && !selClass) {
      err.textContent = 'Choose your ' + _cl + '.';
      err.style.display = 'block';
      return;
    }
    if (!isRadiant && !selRole) {
      err.textContent = 'Choose your ' + _rl + '.';
      err.style.display = 'block';
      return;
    }
    if (!isRadiant && !selWeapon) {
      err.textContent = 'Choose your weapon.';
      err.style.display = 'block';
      return;
    }
    if (!selColor) {
      err.textContent = 'Choose a color.';
      err.style.display = 'block';
      return;
    }
    ...
    const _defenses = window.ConfigResolver
      ? window.ConfigResolver.calcDefensesFromConfig(stats, {})
      : {
          physDef: 10 + (stats.str || 0) + (stats.spd || 0),
          cogDef: 10 + (stats.int || 0) + (stats.wil || 0),
          spirDef: 10 + (stats.awa || 0) + (stats.pre || 0),
        };
    ...
    const base = {
      ...
      isNPC: false,
      campaignId,
      userId: window.Auth && window.Auth.getCurrentUser() ? window.Auth.getCurrentUser().id : null,
      gender,
      fragments: 0,
      oathStage: 1,
      investiture: 0,
      maxInvestiture: maxInv, // Radiants start using this after First Ideal
      recovDie,
      deflect,
      origin: charOrigin || '',
      motivation: charMotivation || '',
      purpose: charMotivation || '',
      obstacle: charObstacle || '',
      goals: [],
    ...
    if (isRadiant) {
      ...
      const surgeSkills = {};
      (ORDER_SURGES[cls.id] || []).forEach((sid) => {
        surgeSkills[sid] = 0;
      });
      ...
      myChar = {
        ...
        ideal4: cls.ideal4,
        spren: cls.spren,
        sprenDesc: cls.sprenDesc || '',
        sprenAssist: cls.sprenAssist || '',
        surges: ORDER_SURGES[cls.id] || [],
        surgeSkills,
        weaponExpertises: [],
        armorExpertises: [],
        abilities: cls.abilities,
        ...
        bladeDesc,
        investiture: maxInv,
        maxInvestiture: maxInv, // Radiants start with full Investiture
        focus: Math.max(1, 2 + (stats.wil || 0)),
        maxFocus: Math.max(1, 2 + (stats.wil || 0)),
      };
    } else {
      ...
      myChar = {
        ...base,
        isRadiant: false,
        className: selRole.name,
        classId: selRole.id,
        roleId: selRole.id,
        roleName: selRole.name,
        pathName: role.id,
        keyTalent: role.keyTalent,
        keyTalentDesc: role.keyTalentDesc,
        startingSkill: role.startingSkill,
        pathSpecialties: role.specialties || [],
        ...
        abilities: [selWeapon.name],
        spren: null,
        weapon: weaponName,
        weaponId: selWeapon.id,
        weaponLevel: 1,
    ...
    const visibleErr = document.querySelector('.screen.active .err-msg') || document.getElementById('create-err');
    if (visibleErr) {
      visibleErr.textContent = 'Error: ' + e.message;
      visibleErr.style.display = 'block';
    }
...
async function assignNPC(slot) {
  ...
  if (gState.players[slot] && gState.players[slot].isPlaceholder) {
    return;
  } // someone is joining
  isLoading = true;
...
async function removeSlot(slot) {
  ...
  if (sz <= 2) {
    alert('Party must have at least 2 slots.');
    return;
  }
...
function renderLobby() {
  try {
    if (!gState) {
      console.error('renderLobby: gState is null');
      return;
    }
    if (!gState.players) gState.players = new Array(gState.partySize || partySize).fill(null);
    const sz = gState.partySize || partySize;
    ...
    if (subEl)
      subEl.textContent = `${humans} player${humans !== 1 ? 's' : ''} · ${npcs} AI companion${npcs !== 1 ? 's' : ''} · ${empty} slot${empty !== 1 ? 's' : ''} open`;
    const grid = document.getElementById('lobby-slots');
    if (!grid) {
      console.error('renderLobby: lobby-slots not found');
      return;
    }
    ...
    grid.innerHTML = Array.from({ length: sz }, (_, i) => {
      ...
      if (!p) {
        ...
        ${
          amHost && !isPlaying
            ? `<div style="display:flex;flex-direction:column;gap:8px;align-items:center;margin-top:4px;">
    ...
    if (!actions) {
      console.error('renderLobby: lobby-actions not found');
      return;
    }
    ...
    if (el) {
      el.textContent = 'Lobby error: ' + e.message;
...
async function startNow() {
  ...
  const st = document.getElementById('lobby-status');
  if (!allFilled) {
    if (st) st.textContent = 'Fill all slots first — assign NPCs to empty slots.';
    return;
  }
  if (!hasHuman) {
    if (st) st.textContent = 'At least one human player is required.';
    return;
  }
...
async function startCampaign() {
  ...
  await addLog({
    type: 'system',
    who: '',
    text: 'The Radiants have spoken their Oaths. The saga begins in ' + ACTS[0].location + '. The Chronicles begin.',
    choices: [],
  });
  stopLobbyPolling();
  showGameScreen();
...
function switchRightTab(tab) {
  ...
  tabs.forEach((t) => {
    ...
    if (btn) {
      btn.style.background = isActive ? 'var(--bg3)' : 'var(--bg2)';
      btn.style.borderBottomColor = isActive ? 'var(--amber)' : 'transparent';
      btn.style.color = isActive ? 'var(--amber2)' : 'var(--text4)';
    }
...
function renderConsequenceBoard() {
  ...
  if (!wm) {
    el.innerHTML =
      condPanel +
      '<div style="color:var(--text5);font-size:12px;font-style:italic;text-align:center;padding:12px;">No discoveries yet.</div>';
    return;
  }
  let html = condPanel;
  ...
  if (myChar) {
    ...
    const focusDots = Array.from(
      { length: maxFocus },
      (_, i) => `<span style="color:${i < focus ? 'var(--amber2)' : 'var(--border2)'};">◈</span>`
    ).join('');
  ...
  if (!html) {
    html =
      '<div style="color:var(--text5);font-size:12px;font-style:italic;text-align:center;padding:12px;">Explore to uncover the world...</div>';
  }
  el.innerHTML = html;
}
  ...
  if ((wm.choices || []).length)
    parts.push(
      'Recent choices: ' +
        wm.choices
          .slice(-3)
          .map((ch) => `Turn ${ch.turn}: ${ch.summary}`)
          .join(' | ')
    );
...
If nothing notable happened, return {}`;
    ...
    /* silent fail — world memory is enhancement not core */
  }
}

function openingPrompt() {
  const sz = gState.partySize || partySize;
  const names = gState.players
    .slice(0, sz)
    .map((p) => `${p.name} the ${p.className}${p.isNPC ? ' (NPC — acts via D4 dice, no AI decisions)' : ' (human player)'}`)
    .join(', ');
  const a1 = ACTS[0],
    a2 = ACTS[1],
    a3 = ACTS[2];
...
async function refreshGame() {
  ...
  if (gState.phase === 'playing') {
    if (gState.beatsUntilCombat == null)
      gState.beatsUntilCombat = COMBAT_BEATS_MIN + Math.floor(Math.random() * (COMBAT_BEATS_MAX - COMBAT_BEATS_MIN + 1));
  ...
  if (gState && !gState.lastGM) {
    const lg = [...log].reverse().find((e) => e.type === 'gm');
    if (lg) gState.lastGM = { text: lg.text, choices: lg.choices, ts: lg.ts };
  }
...
function renderPartyStrip() {
  ...
  (gState.players || []).slice(0, sz).forEach((p, i) => {
    ...
    const newText =
      ...
      (p.maxFocus || 3) +
      ' Focus' +
      (_hasMP && p.maxInvestiture ? ' · ✦ ' + (p.investiture || 0) + '/' + p.maxInvestiture + ' ' + _mpLbl : '') +
      (p.deflect ? ' · DEF ' + p.deflect : '') +
      (p.nextRollAdvantage === 'advantage' ? ' ▲ ADV' : p.nextRollAdvantage === 'disadvantage' ? ' ▼ DIS' : '') +
      (p.conditions && Object.keys(p.conditions).some((k) => p.conditions[k]) ? ' ⚠' : '');
...
function addActionLog(name, action, roll, hpMsg, total) {
  ...
  const success =
    displayRoll >= 18 ? 'CRIT!' : displayRoll >= 14 ? 'SUCCESS' : displayRoll >= 10 ? 'PARTIAL' : displayRoll >= 6 ? 'FAILED' : 'FUMBLE';
...
function renderActionLog() {
  ...
  const log = gState.actionLog || [];
  if (!log.length) {
    el.innerHTML = '<div style="color:var(--text5);font-size:12px;font-style:italic;text-align:center;padding:12px;">No actions yet</div>';
    return;
  }
  // CRIT!   #C9A84C  gold
  // SUCCESS #28A87A  teal
...
async function generateTLDR(storyText) {
  ...
  const el = document.getElementById('tldr-text');
  if (!el) return;
  ...
  try {
    const res = await fetch(PROXY_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: 'Summarise this RPG story beat in exactly 2 punchy sentences. Plain text only, no formatting:\n\n' + storyText,
          },
        ],
      }),
    });
    ...
    if (lang === 'th' && summary) {
      const tres = await fetch(PROXY_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: 'claude-haiku-4-5-20251001',
          max_tokens: 200,
          messages: [
            { role: 'user', content: 'Translate to Thai, keep proper names in English. Return only the translation:\n\n' + summary },
          ],
        }),
      });
...
const DND_IMG_POOL = [...Object.values(DND_CLASS_IMG_MAP), ...Object.values(DND_BG_IMG_MAP)];

function getSprenSVG(classId, bond, isHero, roleName) {
  const sys = (gState && gState.system) || 'stormlight';

  // ── UNIVERSAL: Check class imgUrl from SystemData first (works for ALL systems) ──
  const _classes = (window.SystemData && window.SystemData.classes) || [];
  const _cls = _classes.find((c) => c.id === classId);
  if (_cls && _cls.imgUrl) {
    return `<img class="spren-img" src="${_cls.imgUrl}" alt="${_cls.name || classId}" style="border-radius:8px;object-fit:cover;">`;
  }
  // Also check heroRoles for background-path characters
  if (isHero) {
    const _roles = (window.SystemData && window.SystemData.heroRoles) || [];
    const _role = _roles.find((r) => r.id === classId || r.name === roleName);
    if (_role && _role.imgUrl) {
      return `<img class="spren-img" src="${_role.imgUrl}" alt="${_role.name || classId}" style="border-radius:8px;object-fit:cover;">`;
    }
  }
  ...
  if (sys === 'dnd5e') {
    ...
    if (isHero) {
      ...
      const bgImg = DND_BG_IMG_MAP[bgKey] || DND_BG_IMG_MAP[bgKey.toLowerCase()] || DND_BG_IMG_MAP[(classId || '').toLowerCase()];
...
async function skipCombatTurn(playerName) {
  ...
  renderCombatActions();
  const sz = gState.partySize || partySize;
  const allSubmitted = gState.players
    .slice(0, sz)
...
function handleSkipTop() {
  ...
  if (gState.combatMode) {
    ...
    if (isSelfTurn() || awayMode) {
      ...
      const notSubmitted = gState.players
        .slice(0, sz)
...
function renderParty() {
  ...
  s.innerHTML = gState.players
    ...
    .map((p, i) => {
      if (!p)
        return `<div class="ppip"><div class="ppip-top"><div class="ppip-dot" style="background:var(--border2);"></div><span class="ppip-name" style="color:var(--text5);">Empty</span></div></div>`;
...
function renderStory(log) {
  ...
  if (!dlog.length) {
    el.innerHTML =
      '<div style="text-align:center;color:var(--text4);font-style:italic;padding:2rem 0;"><span data-tr>Waiting for all players to join...</span></div>';
    return;
  }
    ...
    .map((e) => {
      if (e.type === 'gm') {
        const raw = e.text
          ...
          .trim();
        const badge = raw.includes('[COMBAT]')
          ? '<span class="event-badge badge-combat">Combat</span>'
          : raw.includes('[DISCOVERY]')
            ? '<span class="event-badge badge-discovery">Discovery</span>'
            : raw.includes('[DECISION]')
              ? '<span class="event-badge badge-decision">Decision</span>'
              : '';
        const clean = raw
      ...
      if (e.type === 'player') {
        const p = gState && gState.players.find((x) => x && x.name === e.who);
        return `<div class="entry-player"><div class="entry-pname" style="color:${p ? p.color : 'var(--text3)'};">${e.who}</div>${e.text}</div>`;
      }
...
function setBottomLoading() {
  ...
  if (tb) {
    ...
    tb.classList.add('thinking-active');
  }
}
...
function setBottomReadGate(choices) {
  ...
  if (storyEl) {
    storyEl.classList.remove('story-reveal');
    void storyEl.offsetWidth;
    storyEl.classList.add('story-reveal');
  }
  const glyph = document.querySelector('.chronicle-title');
  if (glyph) {
    glyph.classList.remove('glyph-flare');
  ...
  if (mine) {
    ...
    if (choices && choices.length) {
      ...
      if (lang === 'th') {
        ...
        Promise.all(displays.map((d) => translateToThai(d)))
          .then((translated) => {
            ch.querySelectorAll('.achoice').forEach((btn, i) => {
              ...
              btn.appendChild(document.createTextNode(translated[i] || displays[i]));
...
async function handleNPC(log) {
  ...
  const opts =
    lastGM && lastGM.choices && lastGM.choices.length
      ? lastGM.choices
      : ['Takes a defensive stance', 'Scouts the area', 'Searches for allies', 'Draws weapon and advances'];
  ...
  setTimeout(() => {
    setBottomFromState(freshLog);
    maybeTranslateStory();
    if (gState && gState.lastGM && gState.lastGM.text) generateTLDR(gState.lastGM.text);
  }, 150);
  ...
  if (_npcGMText) {
    ...
    gState.players
      .slice(0, sz_oath)
      .filter((p) => p && (window.ConfigResolver ? window.ConfigResolver.hasClassPath(p) : p.isRadiant) && !p.isNPC)
      .forEach((p) => {
...
function npcPrompt(action, npc, from) {
  const m = gState.totalMoves || 0;
  const act = getAct(m);
  const sz = gState.partySize || partySize;
  const next = gState.players[gState.turn];
  const party = gState.players
    .slice(0, sz)
    .map((p) => (p ? `${p.name}(${p.className}${p.isNPC ? ' NPC' : ''} HP:${p.hp}/${p.maxHp})` : '?'))
    .join(' | ');
...
async function onSubmitAction() {
  ...
  if (!action) {
    errEl.textContent = 'Choose or describe an action first.';
  ...
  if (plotOrNatural) {
    gState.lastPlotEffect = {
      ...
      actor: myChar.name,
      bucket,
      effect: plotOrNatural.type === 'opportunity' ? getOpportunityEffect(bucket, myChar.name) : getComplicationEffect(bucket, myChar.name),
    };
  } else {
...
function getActionBucket(a) {
  ...
  if (window.ActionEngine) {
    ...
    return {
      ...
      restAction: resolved.restAction,
      stormlightAction: resolved.stormlightAction,
    };
  }
  ...
  const _magicRe = new RegExp(
    'breathe.*' +
      _magicN5.toLowerCase() +
      '|inhale.*' +
      _magicN5.toLowerCase() +
      '|draw.*' +
      _magicN5.toLowerCase() +
  ...
  if (_magicRe.test(t)) return { bucket: 'skill', stat: STAT_KEYS[4] || 'awa', skill: 'perception', stormlightAction: 'breathe' };
  const _enhRe = new RegExp(_magicN5.toLowerCase() + '.*enhance|enhance.*str|enhance.*power', 'i');
  if (_enhRe.test(t)) return { bucket: 'skill', stat: STAT_KEYS[3] || 'wil', skill: 'discipline', stormlightAction: 'enhance' };
  const _regRe = new RegExp(_magicN5.toLowerCase() + '.*regen|regenerate.*' + _magicN5.toLowerCase(), 'i');
  if (_regRe.test(t)) return { bucket: 'heal', stat: STAT_KEYS[4] || 'awa', skill: 'progression', stormlightAction: 'regenerate' };
  ...
  const isRevive =
    /revive|stabilize|pull.*back|save.*downed|rouse|wake.*up|bring.*back|resuscitate/.test(t) &&
    /ally|companion|downed|fallen|hobber|friend|party/.test(t);
  const isHeal =
    /\[heal\]|heal|regrow|mend|restore|tend|bandage|cure|knit|medicine|patch.*wound|stitch|treat.*injur|fix.*wound|channel.*into|mend.*bone|check.*breathing|check.*wound|check.*on|pull.*behind.*shelter|drag.*to.*safety|yank.*behind|nurse|aid.*allie|triage|infuse.*with|pour.*energy|ease.*pain|soothe/.test(
      t
    );
  ...
  if (
    /defend|protect|shield|block|parry|stance|barrier|brace|guard|hunker|cover.*allie|hold.*line|take.*position|hold.*ground|fortif|entrench|dig.*in|plant.*feet|stand.*firm|sentinel|interpose|screen|ward/.test(
      t
    )
  )
    return { bucket: 'defend', stat: 'pre', skill: 'athletics' };
  // ── Cognitive skills ──
  ...
  if (/surge|transform|summon|conjure|transmute|pattern|soulcast|lash/.test(t))
    return { bucket: 'surge', stat: 'int', skill: 'transformation' };
...
function shouldRaisePricing(total, roll, bucket, beatsLeft) {
  ...
  if (isHighStakes) return Math.random() < 0.3;
...
function applyFocusChange(actorName, delta) {
  ...
  gState.players[idx] = p;
  if (myChar && myChar.name === actorName) {
    myChar.focus = p.focus;
    saveMyChar(myChar);
  }
}
...
function getOpportunityEffect(bucket, actor) {
  ...
  if (inCombat) {
    ...
    if (officialOpp.includes('recover 1 focus') && p) {
...
function getComplicationEffect(bucket, actor) {
  ...
  if (inCombat) {
    ...
    if (officialComp.includes('fall Prone') && p) {
      applyCondition(p, 'prone');
    }
    ...
    if (officialComp.includes('lose') && p) {
      applyFocusChange(actor, -1);
    }
    return officialComp;
  }
  ...
  if (
    (bucket === 'surge' || bucket === 'magic' || bucket === 'corruption') &&
    p &&
    (window.ConfigResolver ? window.ConfigResolver.hasClassPath(p) : p.isRadiant)
  ) {
...
function turnPrompt(action, roll, sk, from) {
  ...
  const party = gState.players
    ...
    .map((p) => {
      ...
      const conds = p.conditions
        ? Object.entries(p.conditions)
            .filter(([k, v]) => v)
            .map(([k]) => (CONDITIONS[k] ? CONDITIONS[k].name : k))
        : [];
    ...
    .join(' | ');
  const who = gState.players[from];
  const loc = act.location || act.name;
  const rollDesc =
    roll >= 18
      ? 'CRITICAL SUCCESS — something unexpected and wonderful happens'
      : roll >= 14
        ? 'Strong success — clean, clear outcome'
        : roll >= 10
          ? 'Partial success — they get what they want but something is now complicated or wrong'
          : roll >= 6
            ? 'Failure — what they wanted did not happen, and the world responds'
            : 'CRITICAL FAILURE — something goes badly wrong, raise the stakes hard and immediately';
  const gctx = getGenderContext();
  const mctx = getSprenMemoryContext();
  const wmctx = getWorldMemoryContext();
  const cctx = getCharContext();
...
Fresh ground. Build ${loc} with sensory specificity — what makes this place unlike anywhere else in ${window.SystemData?.gmContext?.worldName || 'this world'}. Introduce details that will matter. No hints at combat yet. Let the world feel vast.`;
  ...
  const actHint =
    m === 59
      ? `\n\nACT TRANSITION: Close out ${loc} emotionally. One sensory detail foreshadowing ${ACTS[1] ? ACTS[1].location : 'what comes next'}.`
      : m === 119
...
function mkRain(gain = 0.06) {
  ...
  const fd = audioCtx.createGain();
  fd.gain.value = gain * 0.08;
  flutter.connect(fd);
  fd.connect(g.gain);
  flutter.start();
  src.connect(hp);
...
function mkThunder() {
  ...
  function strike() {
    ...
    crackFilt.frequency.value = 2000;
    const crackG = audioCtx.createGain();
    crackG.gain.value = 0.25;
    crack.connect(crackFilt);
    crackFilt.connect(crackG);
...
function mkHum() {
  ...
  o2.type = 'sine';
  o2.frequency.value = 220.8;
  const o3 = audioCtx.createOscillator();
  o3.type = 'sine';
  o3.frequency.value = 440.3;
  const g = audioCtx.createGain();
  g.gain.value = 0.018;
...
const AMBIENT_PRESETS = {
  storm: {
    icon: '🌩',
    label: 'STORM',
    desc: 'Howling winds, rain, and thunder',
    w1: [380, 0.11, 0.07, -0.3],
    w2: [780, 0.07, 0.12, 0.3],
    w3: [180, 0.14, 0.04, 0],
    w4: [1200, 0.04, 0.18, -0.6],
    rain: 0.055,
    thunder: true,
    hum: true,
  },
  forge: {
    icon: '🔥',
    label: 'FORGE',
    desc: 'Crackling fire, bellows, radiant heat',
    w1: [120, 0.06, 0.03, 0],
    w2: [2200, 0.03, 0.08, -0.4],
    w3: [1800, 0.02, 0.1, 0.4],
    w4: [90, 0.05, 0.02, 0],
    rain: 0.02,
    hum: true,
  },
  ocean: {
    icon: '🌊',
    label: 'OCEAN',
    desc: 'Rolling waves, sea breeze, distant gulls',
    w1: [180, 0.1, 0.015, 0],
    w2: [420, 0.06, 0.02, 0.3],
    w3: [90, 0.12, 0.01, -0.2],
    w4: [3000, 0.015, 0.04, 0.5],
    rain: 0.015,
    hum: true,
  },
  forest: {
    icon: '🌲',
    label: 'FOREST',
    desc: 'Wind through leaves, birdsong, rustling',
    w1: [600, 0.05, 0.04, -0.3],
    w2: [1400, 0.03, 0.06, 0.4],
    w3: [200, 0.04, 0.02, 0],
    w4: [2800, 0.02, 0.08, -0.5],
    rain: 0.01,
    hum: true,
  },
  dungeon: {
    icon: '🕯',
    label: 'DUNGEON',
    desc: 'Dripping water, echoing stone, distant moans',
    w1: [60, 0.08, 0.01, 0],
    w2: [3200, 0.02, 0.12, -0.6],
    w3: [1600, 0.015, 0.08, 0.5],
    w4: [45, 0.06, 0.01, 0],
    rain: 0.03,
  },
  tavern: {
    icon: '🍺',
    label: 'TAVERN',
    desc: 'Warm fire, murmuring crowd, clinking mugs',
    w1: [160, 0.07, 0.03, 0],
    w2: [800, 0.03, 0.05, -0.3],
    w3: [1200, 0.02, 0.06, 0.4],
    w4: [100, 0.04, 0.02, 0],
    rain: 0.02,
    hum: true,
  },
  desert: {
    icon: '🏜',
    label: 'DESERT',
    desc: 'Dry wind, shifting sand, scorching silence',
    w1: [500, 0.09, 0.06, -0.2],
    w2: [1100, 0.04, 0.09, 0.3],
    w3: [250, 0.06, 0.03, 0],
    w4: [2000, 0.02, 0.05, 0.5],
  },
  arctic: {
    icon: '❄',
    label: 'ARCTIC',
    desc: 'Howling blizzard, creaking ice, frozen silence',
    w1: [320, 0.13, 0.08, -0.4],
    w2: [900, 0.08, 0.14, 0.3],
    w3: [150, 0.1, 0.05, 0],
    w4: [1500, 0.05, 0.2, -0.5],
    rain: 0.04,
  },
  jungle: {
    icon: '🌴',
    label: 'JUNGLE',
    desc: 'Humid buzz, insects, dripping canopy',
    w1: [400, 0.04, 0.05, 0],
    w2: [2600, 0.03, 0.1, -0.4],
    w3: [3400, 0.025, 0.12, 0.5],
    w4: [180, 0.05, 0.03, 0],
    rain: 0.035,
    hum: true,
  },
  volcanic: {
    icon: '🌋',
    label: 'VOLCANIC',
    desc: 'Deep rumbling, hissing vents, molten heat',
    w1: [55, 0.14, 0.02, 0],
    w2: [40, 0.1, 0.015, -0.2],
    w3: [800, 0.04, 0.06, 0.3],
    w4: [2400, 0.02, 0.08, 0.5],
    rain: 0.025,
    thunder: true,
  },
  cathedral: {
    icon: '⛪',
    label: 'CATHEDRAL',
    desc: 'Reverberant hum, distant bells, sacred stillness',
    w1: [110, 0.03, 0.01, 0],
    w2: [440, 0.02, 0.03, -0.3],
    w3: [660, 0.015, 0.02, 0.3],
    hum: true,
  },
  battlefield: {
    icon: '⚔',
    label: 'BATTLE',
    desc: 'War drums, marching, horns on the wind',
    w1: [80, 0.1, 0.04, 0],
    w2: [260, 0.06, 0.05, -0.3],
    w3: [520, 0.04, 0.03, 0.3],
    w4: [1000, 0.03, 0.07, 0],
    rain: 0.02,
    thunder: true,
  },
  cosmic: {
    icon: '✨',
    label: 'COSMIC',
    desc: 'Ethereal drones, shimmering tones, vast emptiness',
    w1: [60, 0.06, 0.008, 0],
    w2: [220, 0.04, 0.012, -0.4],
    w3: [330, 0.03, 0.015, 0.4],
    hum: true,
  },
  swamp: {
    icon: '🐸',
    label: 'SWAMP',
    desc: 'Bubbling muck, croaking, thick humid drone',
    w1: [140, 0.07, 0.03, 0],
    w2: [1800, 0.025, 0.09, -0.4],
    w3: [2400, 0.02, 0.07, 0.5],
    w4: [70, 0.05, 0.02, 0],
    rain: 0.03,
  },
  clockwork: {
    icon: '⚙',
    label: 'CLOCKWORK',
    desc: 'Mechanical ticking, steam hiss, grinding gears',
    w1: [2000, 0.03, 0.14, -0.3],
    w2: [3000, 0.025, 0.18, 0.3],
    w3: [100, 0.05, 0.03, 0],
    w4: [4000, 0.02, 0.2, -0.5],
    rain: 0.02,
  },
  // ── New presets ──
  cyberpunk: {
    icon: '🌆',
    label: 'NEON',
    desc: 'Electric hum, distant traffic, rain on chrome',
    w1: [100, 0.06, 0.02, 0],
    w2: [3500, 0.02, 0.15, -0.4],
    w3: [1800, 0.03, 0.08, 0.4],
    w4: [60, 0.04, 0.01, 0],
    rain: 0.04,
    hum: true,
  },
  wasteland: {
    icon: '☢',
    label: 'WASTELAND',
    desc: 'Dead wind, distant rumble, crackling static',
    w1: [200, 0.08, 0.04, 0],
    w2: [1500, 0.03, 0.06, -0.3],
    w3: [80, 0.06, 0.02, 0],
    w4: [4500, 0.01, 0.1, 0.5],
  },
  spaceship: {
    icon: '🚀',
    label: 'SHIP',
    desc: 'Engine drone, hull creaks, ventilation hiss',
    w1: [85, 0.07, 0.005, 0],
    w2: [170, 0.04, 0.008, -0.2],
    w3: [2800, 0.015, 0.03, 0.3],
    w4: [50, 0.05, 0.003, 0],
    hum: true,
  },
  underwater: {
    icon: '🫧',
    label: 'DEEP SEA',
    desc: 'Muffled currents, whale song, pressure hum',
    w1: [90, 0.08, 0.01, 0],
    w2: [350, 0.05, 0.015, -0.3],
    w3: [150, 0.06, 0.008, 0.2],
    w4: [40, 0.04, 0.005, 0],
    hum: true,
  },
  ruins: {
    icon: '🏛',
    label: 'RUINS',
    desc: 'Wind through broken columns, settling dust',
    w1: [250, 0.06, 0.03, -0.2],
    w2: [1000, 0.03, 0.05, 0.3],
    w3: [500, 0.04, 0.02, 0],
    w4: [2200, 0.015, 0.04, -0.4],
  },
  market: {
    icon: '🏪',
    label: 'MARKET',
    desc: 'Bustling crowd, hawkers, exotic instruments',
    w1: [300, 0.04, 0.05, 0],
    w2: [1600, 0.03, 0.08, -0.3],
    w3: [900, 0.025, 0.06, 0.4],
    w4: [2200, 0.02, 0.07, -0.4],
    rain: 0.01,
    hum: true,
  },
};
...
function setStormIntensity(type) {
  ...
  const now = audioCtx.currentTime;
  const t = {
    combat: { w1: 0.22, w2: 0.16, w3: 0.25, hum: 0.04 },
    decision: { w1: 0.06, w2: 0.04, w3: 0.08, hum: 0.06 },
    discovery: { w1: 0.09, w2: 0.07, w3: 0.1, hum: 0.08 },
    normal: { w1: 0.12, w2: 0.08, w3: 0.15, hum: 0.02 },
  }[type] || { w1: 0.12, w2: 0.08, w3: 0.15, hum: 0.02 };
...
async function batchTranslate(strings) {
  ...
  if (uncached.length) {
    try {
      ...
      const prompt =
        'Translate each numbered item to Thai. Keep character names, place names, and game-specific terms in English. Return ONLY a numbered list:\n\n' +
      ...
      raw
        ...
        .forEach((line) => {
          ...
          if (m) {
            ...
            if (idx >= 0 && idx < uncached.length) thaiCache[uncached[idx]] = m[2].trim();
          }
        });
    } catch (e) {
      console.warn('batchTranslate failed:', e.message);
    }
  }
...
async function applyThaiToPage() {
  ...
  if (unique.length) {
    ...
    const map = {};
    unique.forEach((t, i) => {
      if (translated[i] && translated[i] !== t) map[t] = translated[i];
    });
    textNodes.forEach((n) => {
  ...
  tagged.forEach((el) => {
    if (!el.getAttribute('data-tr-orig')) el.setAttribute('data-tr-orig', el.textContent.trim());
  });
  const taggedTexts = tagged
    .map((el) => el.getAttribute('data-tr-orig') || el.textContent.trim())
    .filter((t) => t && t.length > 1 && !/[฀-๿]/.test(t));
  if (taggedTexts.length) {
    const taggedTrans = await batchTranslate(taggedTexts);
    tagged.forEach((el, i) => {
      if (taggedTrans[i] && taggedTrans[i] !== taggedTexts[i]) el.textContent = taggedTrans[i];
    });
  }
    ...
    const res = await fetch(PROXY_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: `Translate the following text to Thai. Keep all character names, place names, and game-specific terms in their original English form. Return ONLY the translated text with no explanation or preamble:
