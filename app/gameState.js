...
function _SD() {
  return window.SystemData || window.StormlightSystem || {};
}
...
Object.defineProperties(window, {
  ...
  SPREN_BONDS: {
    get() {
      return _SD().sprenBonds || {};
    },
  },
  HERO_ROLES: {
    get() {
      return _SD().heroRoles || [];
    },
  },
  HERO_WEAPONS: {
    get() {
      return _SD().heroWeapons || [];
    },
  },
  ANCESTRIES: {
    get() {
      return _SD().ancestries || [];
    },
  },
  CULTURES: {
    get() {
  ...
  SINGER_FORMS: {
    get() {
      return _SD().singerForms || {};
    },
  },
  STARTING_KITS: {
    get() {
      return _SD().startingKits || [];
    },
  },
  // Stats & skills
  STAT_KEYS: {
  ...
  SURGE_SCALE: {
    get() {
      return _SD().surgeScale || [];
    },
  },
  ORDER_SURGES: {
    get() {
      return _SD().orderSurges || {};
    },
  },
  CONDITIONS: {
    get() {
      return _SD().conditions || {};
    },
  },
  INJURY_EFFECTS: {
    get() {
      return _SD().injuryEffects || [];
    },
  },
  WEAPONS: {
    get() {
  ...
  ADVERSARY_ROLES: {
    get() {
      return _SD().adversaryRoles || {};
    },
  },
  COMBAT_OPPS: {
    get() {
      return _SD().combatOpps || [];
    },
  },
  COMBAT_COMPS: {
    get() {
      return _SD().combatComps || [];
    },
  },
  // NPC
  NPC_M: {
  ...
  NPC_F: {
    get() {
      return _SD().npcFemale || [];
    },
  },
  NPC_ALL: {
    get() {
      return [...(_SD().npcMale || []), ...(_SD().npcFemale || [])];
    },
  },
  COLORS: {
    get() {
  ...
  NPC_COLORS: {
    get() {
      return _SD().npcColors || [];
    },
  },
  // Lore & flavor
  SPREN_APPEARANCES: {
    get() {
      return _SD().sprenAppearances || {};
    },
  },
  PURPOSES: {
    get() {
  ...
  OBSTACLES: {
    get() {
      return _SD().obstacles || [];
    },
  },
  GEMSTONES: {
    get() {
      return _SD().gemstones || {};
    },
  },
  HOID_LINES: {
    get() {
      return _SD().hoidLines || [];
    },
  },
  // World — generic aliases (ROSHAR_* kept for backward compat)
  LOCATIONS: {
    get() {
      return _SD().locations || [];
    },
  },
  LOCATIONS_ALT: {
    get() {
      return _SD().offworldLocations || [];
    },
  },
  LOCATIONS_SUB: {
    get() {
      return _SD().shadesmarLocations || [];
    },
  },
  LOCATIONS_LEG: {
    get() {
      return _SD().legendaryLocations || [];
    },
  },
  ROSHAR_P: {
    get() {
  ...
  ALL_LOCS: {
    get() {
      return [
        ...(_SD().locations || []),
        ...(_SD().offworldLocations || []),
        ...(_SD().shadesmarLocations || []),
        ...(_SD().legendaryLocations || []),
      ];
    },
  },
  BASE_ACTS: {
  ...
  BLADE_TIERS: {
    get() {
      return _SD().bladeTiers || [];
    },
  },
  BLADE_NAMES: {
    get() {
      return _SD().bladeNames || {};
    },
  },
  WEAPON_PREFIXES: {
    get() {
      return _SD().weaponPrefixes || [];
    },
  },
  WEAPON_SUFFIXES: {
    get() {
      return _SD().weaponSuffixes || [];
    },
  },
  // Progression
  ORDER_OATHS: {
    get() {
      return _SD().orderOaths || {};
    },
  },
  OATH_BONUSES: {
    get() {
      return _SD().oathBonuses || {};
    },
  },
  ADVANCEMENT: {
    get() {
      return _SD().advancement || {};
    },
  },
  ORDER_IDEALS: {
    get() {
      return _SD().orderIdeals || {};
    },
  },
});
...
function _getAttrPoints() {
  return (_SD().charCreation && _SD().charCreation.attributePoints) || 12;
}
function _getAttrMax() {
  return (_SD().charCreation && _SD().charCreation.maxPerAttribute) || 3;
}
...
function applyDeflect(dmg, dmgType, deflect) {
  const isDefl = window.ConfigResolver ? window.ConfigResolver.isDeflectable(dmgType) : ['energy', 'impact', 'keen'].includes(dmgType);
  if (isDefl && deflect > 0) {
...
let ACTS =
  typeof BASE_ACTS !== 'undefined' && BASE_ACTS.length
    ? BASE_ACTS.map((a) => ({ ...a, name: a.tag, location: (_SD().gmContext || {}).worldName || 'the world' }))
    : [];
...
async function checkOathMoment(gmText, player) {
  ...
  const ideal = ORDER_IDEALS[player.classId];
  if (!ideal) return;
  const recentActions = (gState.actionLog || [])
    .slice(0, 6)
    .filter((e) => e.name === player.name)
    .map((e) => e.verb + ' ' + (e.noun || ''));
...
async function progressOath(player) {
  ...
  if (myChar && myChar.name === player.name) {
    myChar = player;
    saveMyChar(player);
  }
...
function getHealAmount(player, stageOverride) {
  ...
  const mult = window.ConfigResolver
    ...
    : player.classId === 'edgedancer'
      ? 1.8
      : player.classId === 'bondsmith'
        ? 0
        : 1;
...
function getBondsmitnPartyHeal(stage) {
  return Math.max(2, Math.floor(stage * 1.5 + 2));
} // 2,3,5,7,9
function getReviveCost(stage) {
  return Math.max(0, 3 - stage);
} // 3,2,1,0,0 fragments
function getReviveHP(stage) {
  return Math.max(1, stage * 2);
} // 0,2,4,6,8 → capped at 1 min
...
function buildCombatChoices(player, combatState) {
  ...
  const healPool = [
    downed.length > 0
      ? `[HEAL] I drop to ${downed[0].name}'s side and channel ${_magicName} into the wound`
      : `[HEAL] I press ${_magicName} into the worst of my injuries and force them to close`,
    downed.length > 0
      ? `[HEAL] I cover ${downed[0].name} and pour everything I have into keeping them alive`
...
async function generateCombatChoices(player) {
  ...
  const enemyDesc =
    enemies
      .map(
        (e) =>
          `${e.name}(${Math.round((e.hp / e.maxHp) * 100)}% HP${e.conditions && Object.keys(e.conditions).length ? ' ' + Object.keys(e.conditions).join(',') : ''} )`
      )
      .join(', ') || 'no enemies standing';
  const partyDesc = gState.players
    .slice(0, sz)
    .filter((p) => p && !p.downed)
    .map((p) => `${p.name}(${Math.round((p.hp / (p.maxHp || 10)) * 100)}%HP)`)
    .join(', ');
  const healOption =
    downed.length > 0
      ? `REQUIRED: One option MUST be [HEAL] to attempt reviving ${downed[0].name}.`
      : anyInjured
        ? `REQUIRED: One option MUST be [HEAL] targeting an injury (estimated +${healAmt}HP).`
        : '';
  const surges = ORDER_SURGES[player.classId] || [];
  const surgeNames = surges
    .map((id) => SURGES.find((s) => s.id === id))
    .filter(Boolean)
    .map((s) => s.name)
    .join(' and ');
  ...
  const prevWarning =
    prevChoices.length > 0
      ? `\nDO NOT use these phrases or approaches from previous rounds: ${prevChoices
          .flat()
          .slice(0, 6)
          .map((c) => c.slice(0, 40))
          .join(' | ')}`
  ...
  const recentLog = (gState.combatLog || [])
    .slice(-2)
    .map((e) => e.text || '')
    .filter(Boolean)
    .join(' ');
  ...
  const _actionTags =
    (_SD().combatActions || []).map((a) => '[' + a.tag + ']').join(', ') || '[ATTACK], [DEFEND], [HEAL], [' + _surgeTag2 + ']';
...
Return ONLY 4 numbered lines. Nothing else.`;
  ...
  try {
    ...
    console.warn('generateCombatChoices failed, using fallback:', e.message);
  }
}
...
const SESSION_ID =
  sessionStorage.getItem('sc_session') ||
  (() => {
