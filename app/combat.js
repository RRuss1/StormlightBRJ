...
function renderEnemies() {
  const p = document.getElementById('enemy-panel');
  const el = document.getElementById('enemy-list');
  if (!p || !el || !gState) return;
  ...
  el.innerHTML = living
    .map((e) => {
      ...
      const col = pct > 60 ? '#c44a28' : pct > 30 ? '#BA7517' : '#888';
      return `<div class="enemy-card"><div class="enemy-name">${e.name}</div><div class="enemy-hp">${e.hp}/${e.maxHp} HP · ${e.type}</div><div style="height:3px;background:var(--border);border-radius:2px;margin-top:4px;overflow:hidden;"><div style="height:100%;width:${pct}%;background:${col};border-radius:2px;transition:width 0.4s;"></div></div></div>`;
    })
    .join('');
}
function renderInitiativeOrder() {
  ...
  if (!p || !gState) return;
  if (gState.combatMode || !gState.combatActive || !(gState.combatOrder || []).length) {
    p.style.display = 'none';
    return;
  }
  ...
  document.getElementById('init-row').innerHTML = (gState.combatOrder || [])
    .map((co) => {
...
function applyRollDamage(roll, actor, st) {
  ...
  if (roll >= 18) {
    ...
    if (target) {
      const d = db.crit + Math.floor(Math.random() * 3) + 1;
      const oldHP = target.hp;
      target.hp = Math.max(0, target.hp - d);
      setTimeout(
        () =>
          animateHPChange(target.name, Math.round((oldHP / target.maxHp) * 100), Math.round((target.hp / target.maxHp) * 100), d, false),
        50
      );
      if (target.hp === 0) {
        actor.fragments += 3;
        checkEquipDrop(actor, st);
      }
      msg = `✦ Crit! ${target.name} -${d}HP${target.hp === 0 ? ' DEFEATED! +3 frags' : ''}  Frags:${actor.fragments}`;
    } else {
      const h = Math.min(2, actor.maxHp - actor.hp);
      actor.hp = Math.min(actor.maxHp, actor.hp + h);
      msg = h > 0 ? `⬆ +${h} HP` : '✦ Critical strike';
    }
  } else if (roll >= 10) {
    if (target) {
      const d = db.hit + Math.floor(Math.random() * 2);
      target.hp = Math.max(0, target.hp - d);
      const chip = Math.floor(Math.random() * 2);
      if (chip > 0) actor.hp = Math.max(1, actor.hp - chip);
      msg = `Hit! ${target.name} -${d}HP${chip > 0 ? ' / ⬇-' + chip + ' HP' : ''}`;
    } else {
      const d = Math.floor(Math.random() * 2);
      if (d > 0) actor.hp = Math.max(1, actor.hp - d);
      msg = d > 0 ? `⬇ -${d} HP` : '';
    }
  } else if (roll >= 6) {
    const d = Math.floor(Math.random() * 3) + 1;
    actor.hp = Math.max(1, actor.hp - d);
    msg = `⬇ -${d} HP`;
  } else {
    const d = Math.floor(Math.random() * 4) + 2;
    actor.hp = Math.max(0, actor.hp - d);
    msg = `💀 -${d} HP${actor.hp === 0 ? ' DOWNED!' : ''}`;
  }
  ...
  if (_healMult > 1 && roll >= 10 && roll < 18 && actor.hp < actor.maxHp) {
  ...
  if (idx >= 0) {
    st.players[idx] = actor;
    if (myChar && myChar.name === actor.name) {
      myChar = actor;
      saveMyChar(actor);
    }
  }
...
function checkEquipDrop(actor, st) {
  const _edCfg2 = window.ConfigResolver
    ? window.ConfigResolver.getEquipDropConfig()
    : { legendaryName: 'Legendary Weapon', armorName: 'Legendary Armor' };
  const roll = Math.random();
  let msg = '';
  if (roll > 0.7 && !actor.shardplate) {
    actor.shardplate = true;
    actor.maxHp = Math.round(actor.maxHp * 1.4);
    actor.hp = Math.min(actor.maxHp, actor.hp + 8);
    msg = ' ⬛ ' + _edCfg2.armorName.toUpperCase() + '!';
  } else if (roll > 0.4 && !actor.shardblade) {
    actor.shardblade = BLADE_NAMES[actor.classId] || _edCfg2.legendaryName;
    actor.bladeLevel = 1;
    msg = ` ⚔ ${actor.shardblade}!`;
  }
  return msg;
}
...
function craftBlade() {
  ...
  const _edCfg3 = window.ConfigResolver
    ? window.ConfigResolver.getEquipDropConfig()
  ...
  myChar.bladeLevel = 1;
  myChar.fragments = (myChar.fragments || 0) - _edCfg3.craftCost;
  saveMyChar(myChar);
  const idx = gState.players.findIndex((p) => p && p.name === myChar.name);
  if (idx >= 0) {
    gState.players[idx] = myChar;
...
function upgradeBlade() {
  ...
  if ((myChar.fragments || 0) < _edCfg4.upgradeCost) {
    alert('Need ' + _edCfg4.upgradeCost + ' ' + _edCfg4.fragmentName + 's.');
    return;
  }
  myChar.fragments = (myChar.fragments || 0) - 5;
  myChar.bladeLevel = tier + 1;
  saveMyChar(myChar);
  const idx = gState && gState.players.findIndex((p) => p && p.name === myChar.name);
  if (idx >= 0) {
    gState.players[idx] = myChar;
    saveState(gState).catch(() => {});
  }
  renderSheet();
  alert('⬆ Blade upgraded to Tier ' + myChar.bladeLevel + ' (' + BLADE_TIERS[myChar.bladeLevel - 1] + ')!');
}
...
function recordSprenMemory(action, roll, turn) {
  ...
  const mem =
    roll >= 18
      ? `Turn ${turn}: ${myChar.name} achieved something extraordinary in ${loc} — "${action.substring(0, 50)}"`
      : roll < 6
        ? `Turn ${turn}: ${myChar.name} failed gravely in ${loc} — "${action.substring(0, 50)}"`
        : `Turn ${turn}: ${myChar.name} made a meaningful choice in ${loc} — "${action.substring(0, 50)}"`;
...
function getCharContext() {
  ...
  if ((myChar.culturalExpertises || []).length) {
    ...
    const cultDesc = myChar.culturalExpertises
      .map((c) => c.expertise)
      .filter(Boolean)
      .join(' | ');
  ...
  const topStats = Object.entries(s)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .map(([k, v]) => `${STAT_FULL[STAT_KEYS.indexOf(k)] || k} ${v}`)
    .join(', ');
  if (topStats) parts.push('Highest attributes: ' + topStats);
  const _charDefs = window.ConfigResolver
    ? window.ConfigResolver.calcDefensesFromConfig(s, {})
    : { physDef: 10 + (s.str || 0) + (s.spd || 0), cogDef: 10 + (s.int || 0) + (s.wil || 0) };
  const _defLabels = ((window.SystemData && window.SystemData.rules && window.SystemData.rules.defenses) || [])
    .map((d) => d.label + ':' + (_charDefs[d.id] || 10))
    .join(' ');
  ...
  if (myChar.conditions && Object.keys(myChar.conditions).length) {
    const activeC = Object.entries(myChar.conditions)
      .filter(([k, v]) => v)
  ...
  if (_isCP && myChar.shardblade) {
    ...
    const bpow =
      blevel >= 5
        ? 'fully manifested ' + _edCfg.legendaryName + ', devastating power'
        : blevel >= 4
          ? _edCfg.legendaryName + ' nearly fully formed'
          : blevel >= 3
            ? _edCfg.legendaryName + ' solidifying, reliable in battle'
            : blevel >= 2
              ? _edCfg.legendaryName + ' flickering but usable'
              : 'nascent ' + _edCfg.legendaryName + ', unstable but deadly';
...
function renderPill() {
  ...
  if (!gState || !pill) return;
  const tb = document.getElementById('thinking-bar');
  if (tb && tb.style.display === 'block') {
    return;
  } // keep showing previous turn during deliberation
...
function startPolling() {
  ...
  if (!window._stormScroll) {
    window.addEventListener(
      'scroll',
      () => {
        if (bottomState === 'human-gated' || bottomState === 'continue') checkScrollEnable();
      },
      { passive: true }
    );
...
function charKey(slot) {
  return 'sc_' + (campaignId || 'default') + '_slot' + (slot != null ? slot : 'x');
}
function saveMyChar(ch) {
  // Write-through: localStorage for instant reload + gState for DB sync
  if (ch && ch.slot != null) localStorage.setItem(charKey(ch.slot), JSON.stringify(ch));
}
function loadMyChar() {
  if (!campaignId || !gState) return null;
  const sz = gState.partySize || partySize;
  const userId = window.Auth && window.Auth.getCurrentUser() ? window.Auth.getCurrentUser().id : null;

  // PRIMARY: find character in gState.players (synced from DB — works on any device)
  for (let i = 0; i < sz; i++) {
    const p = gState.players && gState.players[i];
    if (!p || p.isNPC || p.isPlaceholder) continue;
    // Match by user ID (logged in) or by localStorage slot (legacy/guest)
    if (userId && p.userId === userId) {
      mySlot = i;
      localStorage.setItem(charKey(i), JSON.stringify(p)); // cache locally
      return p;
    }
  }

  // FALLBACK: localStorage match (guest/legacy — single device)
  for (let i = 0; i < sz; i++) {
    try {
...
async function onReset() {
  ...
  try {
    ...
    await saveState({
      ...
      partySize,
      campaignId,
      campaignName: (gState && gState.campaignName) || campaignId,
    });
    gState = null;
    myChar = null;
    renderPSZ();
...
const _GENERIC_HAZARDS = {
  ...
  dungeon: {
    ...
    desc: 'Stones crumble from above.',
    effect: '15% chance each round: random combatant takes 2 damage.',
    mechanic: 'plateauCollapse',
  },
  cave: {
    name: 'Toxic Fumes',
    desc: 'Noxious gas seeps from cracks in the rock.',
    effect: 'Players must resist or lose 1 resource.',
    mechanic: 'cognitiveDrain',
  },
};
...
function applyHazardEffect(hazard, gState) {
  ...
  if (hazard.mechanic === 'plateauCollapse' && Math.random() < 0.15) {
    ...
    if (target) {
      ...
      msgs.push(`⚠ Plateau collapse! ${target.name} -2HP`);
    }
  } else if (hazard.mechanic === 'cognitiveDrain') {
    living.forEach((p) => {
      if ((p.fragments || 0) > 0) {
        p.fragments--;
        msgs.push(`${p.name} -1 frag (cognitive drain)`);
      }
    });
    ...
    const target = living[Math.floor(Math.random() * living.length)];
    if (target) {
      msgs.push(`⚠ Void whispers disrupt ${target.name}'s action`);
    }
  }
  return msgs;
}
...
const _DEFAULT_BOSSES = [
  {
    ...
    type: 'Boss',
    phases: [
      { hp: 40, dmg: 8, atk: 7, desc: 'Radiating malevolent power' },
      { hp: 25, dmg: 11, atk: 9, desc: 'Unleashing devastating attacks' },
      { hp: 12, dmg: 14, atk: 12, desc: 'Final desperate onslaught' },
    ],
  ...
  },
  {
    ...
    type: 'Construct',
    phases: [
      { hp: 45, dmg: 7, atk: 6, desc: 'Stone eyes flickering to life' },
      { hp: 28, dmg: 10, atk: 9, desc: 'Full power, shaking the ground' },
      { hp: 14, dmg: 13, atk: 11, desc: 'Overloading, energy crackling' },
    ],
    drop: 'Ancient Relic',
  },
];
...
async function checkHighstorm() {
  ...
  const party = gState.players
    .slice(0, sz)
    .map((p) => (p ? `${p.name} the ${p.className}` : '?'))
    .join(', ');
...
Tag: [COMBAT]. Under 200 words.`;
  ...
  gState.players
    ...
    .forEach((p) => {
      ...
      if (p.conditions) {
        ['surprised', 'stunned', 'prone', 'slowed'].forEach((k) => {
          delete p.conditions[k];
        });
      }
    });
  ...
  try {
    ...
    const avgBlade =
      gState.players
        .slice(0, sz)
        .filter(Boolean)
        .reduce((a, p) => a + (p.bladeLevel || 0), 0) / sz;
    const loc = (getAct(gState.totalMoves || 0).location || '').toLowerCase();
    // ── Enemy pools — read from active system data + shared pattern library ──
    const sysData = window.SystemData || {};
    const ENEMY_POOLS = sysData.enemyPools || { default: [{ name: 'Enemy', type: 'Unknown', baseHP: 10, dmg: 4, attackBonus: 3 }] };

    // ── Build merged narrative patterns: system-specific + shared (filtered by enabled categories) ──
    function _buildNarrativePatterns() {
      const patterns = [];
      // 1. System-specific patterns first (highest priority)
      if (sysData.enemyPatterns && sysData.enemyPatterns.length) {
        patterns.push(...sysData.enemyPatterns);
      }
      // 2. Shared patterns filtered by system's enabled enemy categories
      const sharedLib = window.SHARED_ENEMY_PATTERNS || {};
      const enabledCats = sysData.enemyCategories || [];
      for (const catId of enabledCats) {
        if (sharedLib[catId]) patterns.push(...sharedLib[catId]);
      }
      return patterns;
    }
    const NARRATIVE_ENEMY_PATTERNS = _buildNarrativePatterns();

    // ── Try to match narrative enemies from GM text ────────────────────────────
    function extractEnemiesFromNarrative(gmText) {
      if (!gmText) return null;
      for (const pattern of NARRATIVE_ENEMY_PATTERNS) {
        const allMatch = pattern.keywords.every((re) => re.test(gmText));
        if (allMatch) return pattern.enemies;
      }
      return null; // no match — use location fallback
    }

    // ── Decide pool: narrative → location fallback ─────────────────────────────
    const narrativeGMText = (gState.lastGM && gState.lastGM.text) || '';
    const narrativeEnemies = extractEnemiesFromNarrative(narrativeGMText);

    let pool, count;
    if (narrativeEnemies) {
      // ── Narrative match: pick 2-3 variants from the matched pool ──
      const sorted = [...narrativeEnemies].sort((a, b) => a.baseHP - b.baseHP);
      const numVariants = Math.min(narrativeEnemies.length, Math.max(2, sz - 1));
      const spread = [];
      const step = Math.max(1, Math.floor(sorted.length / numVariants));
      for (let i = 0; i < numVariants; i++) spread.push(sorted[Math.min(i * step, sorted.length - 1)]);
      count = Math.max(1, Math.min(sz, Math.floor(sz * 0.75) + 1));
      pool = Array.from({ length: count }, (_, i) => spread[i % spread.length]);
      console.log('⟁ Narrative enemies:', pool.map((e) => e.name).join(', '));
    } else {
      // ── Location fallback — use system's enemy pools ─────────────────────
      const poolKeys = Object.keys(ENEMY_POOLS);
      let poolKey = 'default';
      // Try to match location string against pool keys
      ...
      const fullPool = ENEMY_POOLS[poolKey] || ENEMY_POOLS.default || [];
    ...
    const avgPartyHP = livePlayers.length ? livePlayers.reduce((s, p) => s + (p.maxHp || p.hp || 10), 0) / livePlayers.length : 12;
    ...
    if (shouldSpawnBoss(gState)) {
      const _bt = _getBossTemplates();
      const boss = _bt[combatActNum - 1] || _bt[0];
      ...
      gState.combatEnemies = [
        {
          ...
          id: 'boss_' + actNum,
          hp: phase.hp,
          maxHp: boss.phases.reduce((a, p) => a + p.hp, 0),
          currentPhase: 0,
          phaseHP: phase.hp,
          dmg: phase.dmg,
          attackBonus: phase.atk,
          isBoss: true,
          downed: false,
          type: boss.type,
        },
...
function renderCombatParty() {
  const el = document.getElementById('combat-party-col');
  if (!el || !gState) return;
  const sz = gState.partySize || partySize;
  el.innerHTML =
    `<div style="font-family:var(--font-d);font-size:10px;letter-spacing:2px;color:var(--amber2);text-transform:uppercase;margin-bottom:8px;">Your Party</div>` +
    gState.players
      .slice(0, sz)
      .filter(Boolean)
      .map((p) => {
...
function renderCombatEnemies() {
  const el = document.getElementById('combat-enemy-col');
  if (!el || !gState) return;
  const enemies = gState.combatEnemies || [];
  el.innerHTML =
    `<div style="font-family:var(--font-d);font-size:10px;letter-spacing:2px;color:var(--coral2);text-transform:uppercase;margin-bottom:8px;">Enemies</div>` +
    enemies
      .map((e) => {
        const pct = Math.round((e.hp / e.maxHp) * 100);
...
function renderDiceTicker() {
  ...
  el.innerHTML = recent
    .map((round) => {
      ...
      return (
        `<div class="dice-round-header">Round ${round.round}</div>` +
        entries
          .map((e) => {
            const rollClass =
              e.result === 'CRIT'
...
function renderCombatInitiative() {
  const el = document.getElementById('combat-init-strip');
  if (!el || !gState) return;
  const sz = gState.partySize || partySize;
  const combatants = [
    ...gState.players
      .slice(0, sz)
      .filter(Boolean)
      .map((p) => ({ name: p.name, type: 'player', color: p.color, downed: p.downed })),
    ...(gState.combatEnemies || []).map((e) => ({ name: e.name, type: 'enemy', color: 'var(--coral2)', downed: e.downed })),
  ];
  el.innerHTML = combatants
    .map(
      (co) =>
        `<div class="init-pill${co.downed ? ' init-pill-dead' : ''}" style="border:1px solid ${co.type === 'enemy' ? 'rgba(196,74,40,0.4)' : 'rgba(191,161,90,0.3)'};background:var(--bg3);color:${co.type === 'enemy' ? 'var(--coral2)' : 'var(--amber2)'};text-decoration:${co.downed ? 'line-through' : 'none'};">${co.name}</div>`
    )
    .join('');
}
...
function renderCombatActions() {
  ...
  if (iAmHuman && !mySubmitted) {
    ...
    if (!choices) {
      ...
      const skelHTML = skels
        .map((tag, i) => {
          const tagCol = tag === 'HEAL' ? 'var(--teal2)' : tag === 'DEFEND' ? 'var(--amber2)' : 'var(--coral2)';
          const tagBadge =
            '<span style="font-size:9px;padding:1px 6px;border-radius:8px;background:' +
            tagCol +
            '22;color:' +
            tagCol +
            ';">' +
            ...
            '</span>';
          return (
            '<button class="achoice" style="' +
            skeletonStyle +
            '"><span class="achoice-num">Option ' +
            (i + 1) +
            ' ' +
            tagBadge +
            '</span><span style="display:inline-block;width:85%;height:14px;background:var(--border2);border-radius:4px;vertical-align:middle;animation:starGlow 1.5s ease-in-out infinite;"></span></button>'
          );
        })
        .join('');
      ...
      const choiceHTML = choices
        .map((ch, i) => {
          ...
          return (
            '<button class="achoice" onclick="selectCombatAction(this,`' +
            safeDisplay +
            '`,`' +
            tag +
            '`)"><span class="achoice-num">Option ' +
  ...
  <div class="action-tooltip" id="action-tooltip">
    ...
    const stillWaiting = gState.players
      .slice(0, sz)
      .filter((p) => p && !p.isNPC && !p.downed && (!gState.pendingActions || !gState.pendingActions[p.name]))
      .map((p) => p.name);
    html += stillWaiting.length
      ? `<div class="combat-waiting">Waiting for ${stillWaiting.join(', ')}...</div>`
      : '<div class="combat-waiting" style="color:var(--teal2);">✓ All actions ready — resolving...</div>';
  } else if (!iAmHuman) {
...
async function submitCombatAction() {
  ...
  if (custom && custom.value.trim() && !/\[(HEAL|ATTACK|DEFEND|SURGE|SKILL|REVIVE)\]/.test(action)) {
    ...
    if (/^(heal|mend|cure|restore|patch|bandage|nurse|tend|triage|soothe)\b/.test(lower)) action = '[HEAL] ' + action;
      ...
      /^(surge|lash|soulcast|lightweave|elsecall|abrasion|adhesion|cohesion|division|gravitation|illumination|progression|tension|transformation|transportation|cast|spell|cantrip|fireball|magic.missile|sacred.flame|smite|hex|eldritch|invoke|channel)\b/.test(
        lower
      )
    )
      action = '[SURGE] ' + action;
    else if (/^(scout|search|perceive|persuade|negotiate|bluff|charm|sneak|hide|stealth|inspect|investigate|examine)\b/.test(lower))
      action = '[SKILL] ' + action;
    else if (/^(attack|strike|slash|stab|smash|swing|charge|rush|shoot|throw|hit|punch|kick|cleave)\b/.test(lower))
      action = '[ATTACK] ' + action;
    // ── Fallback: scan full text for intent keywords ──
    else if (
      /heal|mend|regrow|restore|tend|bandage|cure|knit|patch.*wound|stitch|treat.*injur|fix.*wound|channel.*into|check.*breathing|check.*wound|check.*on|pull.*behind.*shelter|drag.*to.*safety|yank.*behind|nurse|triage|aid.*allie|infuse.*with|pour.*energy|ease.*pain|soothe|cure.*wounds|healing.*word|lay.*on.*hands|potion/i.test(
        lower
      )
    )
      action = '[HEAL] ' + action;
      ...
      /defend|block|shield|protect|brace|guard|parry|stance|hunker|cover.*allie|hold.*line|take.*position|hold.*ground|fortif|entrench|sentinel|interpose|stand.*firm|plant.*feet|ward/i.test(
        lower
      )
    )
      action = '[DEFEND] ' + action;
    else if (
      /soulcast|lash|illusion|surge|transmute|conjure|lightweave|elsecall|infuse|stormlight.*power|invoke|channel.*surge|cast.*spell|magic.missile|fireball|sacred.flame|burning.hands|thunder|lightning|arcane|divine|smite|hex|eldritch|cantrip|ritual/i.test(
        lower
      )
    )
      action = '[SURGE] ' + action;
    else if (
      /search|scout|perceive|persuade|negotiate|bluff|charm|sneak|hide|stealth|inspect|investigate|examine|look.*around|check.*surround|sense.*danger/i.test(
        lower
      )
    )
      action = '[SKILL] ' + action;
  ...
  try {
    ...
    Object.entries(gState.pendingActions || {}).forEach(([name, action]) => {
      ...
      const phase =
        bucket === 'defend'
          ? 'DEFENSE'
          : bucket === 'heal' || bucket === 'revive'
            ? 'HEAL'
            : bucket === 'surge'
              ? 'OFFENSE' // surges resolve with attacks
              : 'OFFENSE';
      ...
      .forEach((a) => {
        if (a.isEnemy) {
          ...
          if (result.hit) {
            ...
            if (myChar && myChar.name === target.name) {
              myChar = target;
              saveMyChar(target);
            }
          }
          enemyAttackResults.push(
            `${enemy.name} attacked ${target.name}: ${result.hit ? `hit for ${dmg} damage (${target.hp}/${target.maxHp}HP${target.downed ? ' — DOWNED!' : ''})` : 'missed'}`
          );
          roundDice.entries.push({
            name: enemy.name,
            roll: result.roll,
            bonus: null,
            total: null,
          ...
          if (typeof window.Rules !== 'undefined' && target) {
            ...
            const ar = window.Rules.resolveAttack(
              ...
              true
            ); // raiseStakes=true in combat: plot die active each attack
            roll = ar.roll;
            bonus = ar.modifier;
            total = ar.total;
            const oc = ar.outcome;
            const AOUT = window.Rules.ATTACK_OUTCOME;
            ...
            if (oc !== AOUT.MISS) {
              ...
              if (target.hp <= 0) {
                ...
                detail =
                  inj.severity === 'Death'
                    ? `${target.name} FALLS — ${inj.severity}`
                    : `${target.name} FALLS — ${inj.severity} for ${inj.duration}`;
              }
              ...
              if (oc === AOUT.CRIT) {
                ...
                if (_critEff === 'burning' || a.bucket === 'surge') {
                  target.burning = 2;
                  detail = detail || `${target.name} -${dmg}HP ☄ BURNING${weaponName ? ' [' + weaponName + ']' : ''}`;
                } else if (_critEff === 'poisoned') {
                  target.poisoned = 2;
                  detail = detail || `${target.name} -${dmg}HP ☠ POISONED${weaponName ? ' [' + weaponName + ']' : ''}`;
                } else {
                  detail =
                    detail ||
                    `${target.name} -${dmg}HP CRIT [${dmgTypeLabel(dmgType)}${weaponName ? ', ' + weaponName : ''}]${target.downed ? ' ☠' : ''}`;
                }
              } else {
              ...
              if (ar.opportunity) {
                p.focus = Math.min(p.maxFocus || 3, (p.focus || 0) + 1);
              }
              if (ar.complication) {
                p.hp = Math.max(1, p.hp - Math.ceil(Math.random() * 2));
              }
              if (pi >= 0) gState.players[pi] = p;
              if (myChar && myChar.name === a.actor) {
                myChar = p;
                saveMyChar(p);
              }
              ...
              if (roll <= 2) {
                ...
                detail = `fumbled — self -${sd}HP${p.downed ? ' DOWNED' : ''}`;
                result = 'FUMBLE';
              } else {
                detail = 'no effect';
              }
            ...
            total = Math.min(20, Math.max(1, roll + bonus));
            ...
            if (target && total >= 10) {
              ...
              if (mainWeapon && mainWeapon.dmg) {
                ...
                let wDmg = 0;
                for (let _d = 0; _d < numDice; _d++) wDmg += Math.ceil(Math.random() * dieSize);
              ...
              if (target.hp <= 0) {
                ...
                if (inj.severity === 'Death') {
                  detail = `${target.name} FALLS — ${inj.severity} (roll ${inj.roll})`;
                } else {
                  detail = `${target.name} FALLS — ${inj.severity} for ${inj.duration}. ${inj.effect}`;
                }
              }
              if (total >= 18) {
                // Class-specific crit effects: check class data for critEffect
              ...
              const sd = Math.floor(Math.random() * 4) + 2;
              p.hp = Math.max(0, p.hp - sd);
              if (p.hp === 0) p.downed = true;
              if (pi >= 0) gState.players[pi] = p;
              if (myChar && myChar.name === a.actor) {
    ...
    // Clear any leftover defending flag for players who didn't choose DEFENSE this round
    {
      ...
      gState.players.forEach((p, i) => {
        if (p && p.defending && !defendingActors.has(p.name)) {
          p.defending = false;
          gState.players[i] = p;
        }
      });
    ...
    allActions
      .filter((a) => a.phase === 'DEFENSE' && !a.isEnemy)
      .forEach((a) => {
        const p = a.player;
        if (!p || p.downed) return;
        const pi = gState.players.findIndex((x) => x && x.name === a.actor);
    ...
    allActions
      ...
      .forEach((a) => {
        ...
        if (a.bucket === 'revive') {
          ...
          if (total >= 10) {
            const revHP =
              total >= 18 ? getReviveHP(stage) * 2 : total >= 14 ? getReviveHP(stage) : Math.max(1, Math.floor(getReviveHP(stage) / 2));
            downedAlly.hp = revHP;
            downedAlly.downed = false;
            const ai = gState.players.findIndex((x) => x && x.name === downedAlly.name);
            if (ai >= 0) gState.players[ai] = downedAlly;
            if (myChar && myChar.name === downedAlly.name) {
              myChar = downedAlly;
              saveMyChar(downedAlly);
            }
            ...
            roundDice.entries.push({
              ...
              bonus,
              total,
              result: total >= 18 ? 'CRIT' : total >= 14 ? 'HIT' : 'PARTIAL',
              detail,
              isEnemy: false,
            });
          } else {
    ...
    gState.players.slice(0, sz_st).forEach((p, pidx) => {
      ...
      if (p.poisoned > 0) {
        ...
        roundDice.entries.push({
          ...
          total: null,
          result: 'EFFECT',
          detail: `☠ poison ${dmgTypeLabel('vital')} -${tick}HP${p.poisoned > 0 ? ' (' + p.poisoned + 'r)' : ' ✓'}`,
          isEnemy: false,
          phase: 'END',
        });
      }
      if (p.burning > 0) {
        ...
        roundDice.entries.push({
          ...
          total: null,
          result: 'EFFECT',
          detail: `🔥 burn ${dmgTypeLabel('energy')} -${tick}HP${p.burning > 0 ? ' (' + p.burning + 'r)' : ' ✓'}`,
          isEnemy: false,
          phase: 'END',
        });
      }
      if (p.voidCorrupted > 0) {
        applyFocusChange(p.name, -1);
        p.voidCorrupted--;
        roundDice.entries.push({
          name: p.name,
          roll: '-',
          ...
          total: null,
          result: 'EFFECT',
          detail: `👁 Voidlight -1 Focus${p.voidCorrupted > 0 ? ' (' + p.voidCorrupted + 'r)' : ' ✓'}`,
          isEnemy: false,
          phase: 'END',
        });
      }
      ...
      if (p.conditions) {
        if (p.conditions.afflicted) {
          ...
          roundDice.entries.push({
            ...
            total: null,
            result: 'EFFECT',
            detail: `Afflicted ${dmgTypeLabel('energy')} -${finalDmg}HP`,
            isEnemy: false,
            phase: 'END',
          });
        }
    ...
    (gState.combatEnemies || []).forEach((e, idx) => {
      ...
      if (e.burning > 0) {
        const tick = Math.ceil(Math.random() * 2) + 1;
        e.hp = Math.max(0, e.hp - tick);
        if (e.hp <= 0) e.downed = true;
        e.burning--;
        roundDice.entries.push({
          name: e.name,
          roll: '-',
          ...
          total: null,
          result: 'EFFECT',
          detail: `burning -${tick}HP${e.downed ? ' ☠' : ''}`,
          isEnemy: true,
          phase: 'END',
        });
        gState.combatEnemies[idx] = e;
      }
      if (e.poisoned > 0) {
        const tick = Math.ceil(Math.random() * 3) + 1;
        e.hp = Math.max(0, e.hp - tick);
        if (e.hp <= 0) e.downed = true;
        e.poisoned--;
        roundDice.entries.push({
          ...
          total: null,
          result: 'EFFECT',
          detail: `poisoned -${tick}HP${e.downed ? ' ☠' : ''}`,
          isEnemy: true,
          phase: 'END',
        });
        gState.combatEnemies[idx] = e;
    ...
    if (gState.combatHazard) {
      ...
      hazMsgs.forEach((m) =>
        roundDice.entries.push({ name: '⚠ Hazard', roll: '-', bonus: null, total: null, result: 'EFFECT', detail: m, isEnemy: false })
      );
      const hb = document.getElementById('combat-hazard-banner');
      if (hb) {
        hb.style.display = 'block';
        hb.textContent = '⚠ ' + gState.combatHazard.name;
      }
    }
      ...
      if (nextPhase < phases.length) {
        ...
        if (boss.hp <= phaseThreshold && boss.hp > 0) {
          ...
          roundDice.entries.push({
            ...
            total: null,
            result: 'EFFECT',
            detail: `${boss.name} enters phase ${nextPhase + 1}!`,
            isEnemy: false,
          });
    ...
    const playerOutcomes = roundDice.entries
      .filter((e) => !e.isEnemy)
      .map((e) => `${e.name}: ${e.result}${e.detail ? ' (' + e.detail + ')' : ''}`);
...
async function callGM(prompt) {
  ...
  try {
    ...
    if (gState) {
      gState.lastGM = { text: scene2, choices: choices2, ts: new Date().toISOString() };
      await saveAndBroadcast(gState);
    }
    ...
    if (gState) {
      gState.lastGM = { text: errText, choices: [], ts: new Date().toISOString() };
      await saveState(gState);
    }
...
async function callCombatGM(type, playerActions, enemyResults) {
  ...
  const roundContext =
    round === 1
      ? 'opening chaos — everyone finding their footing, no pattern yet'
      : round <= 3
        ? 'the shape of the fight is forming — who is dangerous, who is scared'
        : round <= 5
          ? 'exhaustion and desperation bleeding in — mistakes happening'
          : 'final moments — this ends soon, someone or something will break';
...
Fast, visceral, present tense. No game jargon. No "suddenly". Show emotion through action.` +
    ...
    const prevRoundText = (gState.combatLog || [])
      .slice(-2)
      .map((e) => e.text || '')
      .filter(Boolean)
      .join(' ');
...
All injuries accumulate. Present tense. No game jargon. No "suddenly". Vary sentence lengths.` +
    ...
    const injuries = gState.players
      .slice(0, sz)
      .filter((p) => p && p.injuries && p.injuries.length)
      .map((p) => `${p.name}:${p.injuries[0].effect}`)
      .join(', ');
...
async function exitCombat(won) {
  ...
  try {
    ...
    gState.players.slice(0, sz).forEach((p, i) => {
      ...
      if (p.conditions) {
        Object.keys(p.conditions).forEach((k) => {
...
function combatAftermathPrompt(won) {
  ...
  const party = gState.players
    .slice(0, sz)
    .map((p) => (p ? `${p.name}(${p.className} HP:${p.hp}/${p.maxHp})` : '?'))
    .join(' | ');
...
// ── Slider helpers ──
function _ttsVol() {
  const s = document.getElementById('tts-vol-slider');
  return s ? parseInt(s.value) / 100 : 0.95;
}
function _ttsRate() {
  const s = document.getElementById('rate-slider');
  return s ? parseInt(s.value) / 100 : 0.9;
}
function _ttsPitch() {
  const s = document.getElementById('pitch-slider');
...
function _resolveVoice() {
  ...
  for (const n of prefs) {
    const v = vv.find((x) => x.name === n);
    if (v) return v;
  }
  return vv.find((x) => x.lang === 'en-GB' && x.localService) || vv.find((x) => x.lang.startsWith('en')) || vv[0] || null;
}
...
class TTSTextChunker {
  ...
  _try() {
    ...
    if (m1) {
      this.emit(m1[1].trim());
      this.buf = this.buf.slice(m1[0].length);
      return;
    }
    if (this.buf.length > this.MIN) {
      ...
      if (m2) {
        this.emit(m2[1].trim());
        this.buf = this.buf.slice(m2[0].length);
        return;
      }
    }
...
class WebSpeechEngine {
  ...
  push(tok) {
    if (this._active && !this._cancel) this._chunker.push(tok);
  }
  flush() {
    if (this._active && !this._cancel) this._chunker.flush();
  }
  stop() {
    this._cancel = true;
    this._active = false;
  ...
  _next() {
    if (this._cancel || !this._queue.length) {
      this._busy = false;
      return;
    }
    this._busy = true;
...
let _engine = null;
function _getEngine() {
  if (!_engine || !_engine._active) _engine = new WebSpeechEngine();
  return _engine;
}
...
function setVoice(voiceVal) {
  ...
  if (voiceActive) {
    stopSpeaking();
    setTimeout(speakStory, 100);
  }
}

function loadVoicePreference() {
  updateVoiceBtn(); // sync button icon to persisted state on load
  const stored = localStorage.getItem('sc_voice');
  const sel = document.getElementById('voice-select');
  if (stored && sel) {
    sel.value = stored;
