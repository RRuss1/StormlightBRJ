...
async function authRegister(username, email, password, displayName){
  const data = await _json('/db/auth/register','POST',{ username, email:email||undefined, password, displayName:displayName||username });
...
async function initAuth(){
  const t = _token();
  if(!t){ _currentUser=null; renderAuthUI(); return; }
  try {
    await authGetMe();
...
function _injectModal(){
  ...
  div.id = 'auth-modal';
  div.hidden = true;
  div.onclick = function(e){ if(e.target===this) hideAuthModal(); };
  div.innerHTML = `
<div class="auth-box">
...
function _bindModalEvents(){
  ...
  _on('auth-login-pass','keydown', e=>{ if(e.key==='Enter') _doLogin(); });

  // register
  _on('auth-reg-btn','click', _doRegister);
  _on('auth-reg-pass2','keydown', e=>{ if(e.key==='Enter') _doRegister(); });

  // forgot
  _on('auth-forgot-btn','click', _doForgot);
  _on('auth-forgot-email','keydown', e=>{ if(e.key==='Enter') _doForgot(); });
}
